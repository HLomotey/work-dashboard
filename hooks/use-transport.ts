...
import { TripStatus, VehicleStatus } from "@/lib/types/transport";
import type {
  Vehicle,
  Trip,
  TripPassenger,
...
export function useVehicles(filters?: TransportFilters) {
  ...
  const fetcher = useCallback(async () => {
    ...
    if (filters?.search) {
      query = query.or(
        `make.ilike.%${filters.search}%,model.ilike.%${filters.search}%,registration.ilike.%${filters.search}%`
      );
    }
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(["vehicles", filters], fetcher);

  const createVehicle = useCallback(
    async (vehicleData: CreateVehicle) => {
  ...
  const updateVehicle = useCallback(
    async (id: string, updates: UpdateVehicle) => {
      const { data, error } = await supabase
        .from("vehicle")
...
export function useTrips(filters?: TransportFilters) {
  ...
  const fetcher = useCallback(async () => {
    ...
    const tripsWithMetrics = (data || []).map((trip) => ({
      ...trip,
      loadFactor: trip.vehicle
        ? (trip.passenger_count / trip.vehicle.capacity) * 100
        : 0,
      costPerPassenger:
        trip.cost && trip.passenger_count > 0
          ? trip.cost / trip.passenger_count
          : 0,
    })) as TripWithDetails[];
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(["trips", filters], fetcher);

  const createTrip = useCallback(
    async (tripData: CreateTrip) => {
      ...
      if (tripData.passengerCount > vehicle.capacity) {
        throw new Error(
          `Passenger count exceeds vehicle capacity (${vehicle.capacity})`
        );
      }
  ...
  const updateTrip = useCallback(
    async (id: string, updates: UpdateTrip) => {
      const { data, error } = await supabase
        .from("trips")
        .update({ ...updates, updated_at: new Date().toISOString() })
        .eq("id", id)
  ...
  const completeTrip = useCallback(
    async (id: string, actualDistance?: number, actualCost?: number) => {
...
export function useTripPassengers(tripId?: string) {
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(tripId ? ["trip_passengers", tripId] : null, fetcher);

  const addPassenger = useCallback(
    async (passengerData: CreateTripPassenger) => {
      // Check trip capacity
      const { data: trip } = await supabase
        .from("trips")
        .select(
  ...
  const updatePassenger = useCallback(
    async (id: string, updates: UpdateTripPassenger) => {
      const { data, error } = await supabase
        .from("trip_passengers")
        .update(updates)
        .eq("id", id)
...
export function useTransportAnalytics(dateRange?: { start: Date; end: Date }) {
      ...
      totalTrips > 0
        ? completedTrips.reduce((sum, t) => {
            const vehicle = vehicles?.find(
              (v) => v.id === (t as any).vehicle_id
            );
            return (
              sum + (vehicle ? (t.passenger_count / vehicle.capacity) * 100 : 0)
            );
          }, 0) / totalTrips
        : 0;
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(["transport_analytics", dateRange], fetcher);
...
export function useVehicleUtilization(
  ...
  const fetcher = useCallback(async (): Promise<VehicleUtilization[]> => {
    ...
    const utilizationData = await Promise.all(
      vehicles.map(async (vehicle) => {
        ...
        const totalDistance =
          trips?.reduce((sum, t) => sum + (t.distance || 0), 0) || 0;
        const totalPassengers =
          trips?.reduce((sum, t) => sum + t.passenger_count, 0) || 0;
        ...
        const averageLoadFactor =
          tripsCount > 0
            ? trips!.reduce(
                (sum, t) => sum + (t.passenger_count / vehicle.capacity) * 100,
                0
              ) / tripsCount
            : 0;
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(["vehicle_utilization", vehicleId, dateRange], fetcher);
...
export function useRouteAnalytics(dateRange?: { start: Date; end: Date }) {
  ...
  const fetcher = useCallback(async (): Promise<RouteAnalytics[]> => {
    ...
    const routeAnalytics = Array.from(routeMap.entries()).map(
      ([route, routeTrips]) => {
        ...
        const averagePassengers =
          tripsCount > 0 ? totalPassengers / tripsCount : 0;
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(["route_analytics", dateRange], fetcher);
